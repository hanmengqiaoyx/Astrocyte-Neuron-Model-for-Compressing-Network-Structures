...
if __name__ == '__main__':
    ...
    for epoch in range(start_epoch, start_epoch+args.max_epoch):
        ...
        if lr0 < 5e-3 and lr1 < 5e-3 or epoch == args.max_epoch-1:
            ...
            plt.savefig("ResNet56_First_CE.jpg")
            plt.figure(2)
            plt.subplot(2, 2, 1)
            plt.xlabel('epoch')
            plt.ylabel('train loss')
            ...
            plt.savefig("ResNet56_Second_CE.jpg")
            plt.show()
            print('OVER')
            break
        else:
