    ...
    def forward(self, input, epoch):
        if self.training:
            if epoch >= 2 and epoch % 2 == 0:
                ...
                layer40 = F.relu(self.dw_bn4(nn.functional.conv2d(layer31, self.dw_weights4, stride=1, padding=1, bias=None)))  # [1, 128, 28, 2]
                ...
                layer_10 = F.relu(self.bn0(nn.functional.conv2d(layer92, self.weights0, stride=1, padding=1, bias=None)))
                layer11 = F.relu(self.row_bn0(nn.functional.conv2d(layer_10, self.row_weights0, stride=1, bias=None)))
                layer_out = F.relu(self.row_bn1(nn.functional.conv2d(layer11, self.row_weights1, stride=1, bias=None)))  # [1, 1, 456, 1]
                gate = layer_out.view(self.in_features, 1)
            elif epoch >= 2 and epoch % 2 != 0:
                ...
                layer30 = F.relu(self.dw_bn3(nn.functional.conv2d(layer21, self.dw_weights3, stride=1, padding=1, bias=None)))
                layer31 = self.maxpool1(layer30)
                layer40 = F.relu(self.dw_bn4(nn.functional.conv2d(layer31, self.dw_weights4, stride=1, padding=1, bias=None)))  # [1, 128, 28, 1]
                layer60 = F.relu(self.up_bn00(nn.functional.conv_transpose2d(layer40, self.up_sample0, stride=2, bias=None)))
                layer61 = torch.cat((layer30, layer60), dim=1)
                layer62 = F.relu(self.up_bn01(nn.functional.conv2d(layer61, self.up_weights0, stride=1, padding=1, bias=None)))
                layer70 = F.relu(self.up_bn10(nn.functional.conv_transpose2d(layer62, self.up_sample1, stride=2, bias=None)))
